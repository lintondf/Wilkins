...
public class BlockTest {
	...
	public void testBlock() {
		...
		assertTrue( block.getContents() != null && block.getContents().length == AbstractBlock.BLOCK_SIZE );
		assertTrue( Arrays.equals(block.getContents(), new byte[AbstractBlock.BLOCK_SIZE]));
	}

	@Test
	public void testBlockBlock() {
		Random random = new Random();
		AbstractBlock block = new Block();
		byte[] values = new byte[512];
		random.nextBytes(values);
		Block b2 = new Block( values );
		Block b3 = new Block( b2 );
		assertTrue( b3.getCount() == values.length );
		assertTrue( b3.getContents() != null && b3.getContents().length == AbstractBlock.BLOCK_SIZE );
	...
	public void testBlockByteArray() {
		...
		assertTrue( b2.getContents() != null && b2.getContents().length == AbstractBlock.BLOCK_SIZE );
	...
	public void testPad() {
		...
		int n = AbstractBlock.BLOCK_SIZE-values.length;
		assertTrue( Arrays.equals(values, Arrays.copyOf(block.getContents(), values.length)));
		assertTrue( Arrays.equals(new byte[n], Arrays.copyOfRange(block.getContents(), values.length, AbstractBlock.BLOCK_SIZE)));
		block.pad();
		assertTrue( Arrays.equals(values, Arrays.copyOf(block.getContents(), values.length)));
	...
	public void testXorBlock() {
		Block b1 = new Block();
		b1.setCount(AbstractBlock.BLOCK_SIZE);
		Arrays.fill(b1.getContents(), (byte) 0x01 );
		Block b2 = new Block();
		b2.setCount(AbstractBlock.BLOCK_SIZE);
		Arrays.fill(b2.getContents(), (byte) 0x03 );
		AbstractBlock b3 = b2.xor(b1);
		assertTrue( b2.getCount() == AbstractBlock.BLOCK_SIZE );
